...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Could not connect to database. Check your Supabase credentials in supabaseClient.js"
      );
    } finally {
      setLoading(false);
    }
  ...
  async function addProperty() {
    if (!newProp.title || !newProp.address || !newProp.price) {
      flash("Title, Address, and Price are required.");
      return;
    }
    setSaving(true);
    const row = {
    ...
    return (
      <div style={{ ...S.root, display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
        <div style={{ textAlign: "center", color: "#8a7560" }}>
          ...
          <div style={{ fontSize: 15, letterSpacing: 3 }}>Loading EstateDesk...</div>
        </div>
      </div>
    );
  }
      ...
      <main style={S.main}>
        {tab === "Listings" && (
          <div>
            <div style={S.filterBar}>
              ...
              <button onClick={loadAll} style={S.refreshBtn}>&#8635;</button>
            </div>
            {properties.length === 0 && (
              ...
              {filtered.map((p) => (
                <div key={p.id} style={S.card} onClick={() => setSelected(p)}>
                  <div style={{ position: "relative" }}>
                    ...
                    <span style={{ ...S.badge, background: STATUS_STYLE[p.status]?.bg, color: STATUS_STYLE[p.status]?.text }}>{p.status}</span>
              ...
              {buyers.map((b) => (
                <div key={b.id} style={S.buyerCard}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={S.buyerDetail}>Budget: <strong>{b.budget ? fmt(b.budget) : "Not specified"}</strong> | Looking for: <strong>{b.interest}</strong></div>
        ...
        {tab === "Add Property" && (
          <div style={S.formWrap}>
            <div style={S.sectionTitle}>Add New Property</div>
            <div style={S.formGrid}>
              <Field label="Title" value={newProp.title} onChange={(v) => setNewProp((p) => ({ ...p, title: v }))} placeholder="e.g. Sunset Villa" />
              <Field label="Address" value={newProp.address} onChange={(v) => setNewProp((p) => ({ ...p, address: v }))} placeholder="123 Main St, City" />
              <Field label="Price ($)" value={newProp.price} onChange={(v) => setNewProp((p) => ({ ...p, price: v }))} placeholder="e.g. 750000" type="number" />
              <SelField label="Type" value={newProp.type} onChange={(v) => setNewProp((p) => ({ ...p, type: v }))} options={["House","Condo","Apartment","Land"]} />
              ...
              <Field label="Sqft" value={newProp.sqft} onChange={(v) => setNewProp((p) => ({ ...p, sqft: v }))} placeholder="e.g. 2200" type="number" />
              <SelField label="Status" value={newProp.status} onChange={(v) => setNewProp((p) => ({ ...p, status: v }))} options={["Available","Under Offer","Sold"]} />
              <Field label="Photo URL" value={newProp.image} onChange={(v) => setNewProp((p) => ({ ...p, image: v }))} placeholder="https://..." span />
              <div style={{ ...S.field, gridColumn: "1 / -1" }}>
                <label style={S.label}>Description</label>
                <textarea value={newProp.description} onChange={(e) => setNewProp((p) => ({ ...p, description: e.target.value }))}
                  placeholder="Describe the property..." style={{ ...S.input, height: 100, resize: "vertical" }} />
              </div>
            </div>
        ...
        {tab === "Buyer Inquiry" && (
          <div style={S.formWrap}>
            ...
            <div style={S.formGrid}>
              ...
              <Field label="Email" value={contactForm.email} onChange={(v) => setContactForm((p) => ({ ...p, email: v }))} placeholder="jane@email.com" type="email" />
              <Field label="Phone" value={contactForm.phone} onChange={(v) => setContactForm((p) => ({ ...p, phone: v }))} placeholder="555-000-1234" />
              <Field label="Budget ($)" value={contactForm.budget} onChange={(v) => setContactForm((p) => ({ ...p, budget: v }))} placeholder="e.g. 600000" type="number" />
              ...
              <div style={{ ...S.field, gridColumn: "1 / -1" }}>
                ...
                <textarea value={contactForm.notes} onChange={(e) => setContactForm((p) => ({ ...p, notes: e.target.value }))}
                  placeholder="e.g. Need 3+ beds, good school district..." style={{ ...S.input, height: 100, resize: "vertical" }} />
              </div>
            </div>
        ...
        <div style={S.overlay} onClick={() => setSelected(null)}>
          <div style={S.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={S.modalBody}>
              ...
              {selected.description && <p style={{ color: "#5a4a3a", lineHeight: 1.7, marginBottom: 20, fontSize: 14 }}>{selected.description}</p>}
              <div style={{ borderTop: "1px solid #ede4d8", paddingTop: 16 }}>
                <div style={{ fontSize: 11, color: "#8a7560", marginBottom: 10 }}>Update Status</div>
...
const S = {
  root: { fontFamily: "Georgia, serif", background: "#faf7f2", minHeight: "100vh", color: "#2d1f0e" },
  ...
  navBtn: { background: "none", border: "none", color: "#c4a87a", padding: "13px 18px", cursor: "pointer", fontSize: 13, borderBottom: "3px solid transparent", whiteSpace: "nowrap" },
  navActive: { color: "#f5c87a", borderBottom: "3px solid #f5c87a" },
  ...
  buyerCard: { background: "#fff", borderRadius: 12, padding: "18px 20px", display: "flex", gap: 14, alignItems: "flex-start", border: "1px solid #ede4d8" },
